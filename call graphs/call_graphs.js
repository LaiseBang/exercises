...
//opgaven var at bygge en call graph
...
function createDescriptorDisplay(descriptor) {
  ...
  if (descriptor.children != null) {
    ...
    descriptor.children.forEach(child =>
      ul.appendChild(createDescriptorDisplay(child))
    );
  ...
  while (!match && curNode != null) {
    ...
    if (compareTag(rootDesc, curNode)) {
      ...
      if (firstmatchfound) {
        ...
        if (
          curNode.nextElementSibling != null &&
          compareTag(rootDesc, curNode.nextElementSibling)
        ) {
        ...
        // an element might be missing - check if this one matches the next descriptor
          ...
          rootDesc.nextSibling != null &&
          compareTag(rootDesc.nextSibling, curNode)
        ) {
          missingElement = true;
...
function compareTag(desc, node) {
  ...
  // compare tagname
  if (
    node.nodeName.toLowerCase() == desc.tag.toLowerCase() ||
    (desc.alternative != undefined &&
      node.nodeName.toLowerCase() == desc.alternative.toLowerCase())
  ) {
    compares = true;
